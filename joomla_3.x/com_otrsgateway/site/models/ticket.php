 ...
 */
class OTRSGatewayModelTicket extends JModelLegacy
{
    var $_userID = null;
    ...
    function reply( $id = false, $text = '', $priority = false, $state = false, $token = false )
    {
        ...
        if ( $token )
        {
            $atm =& JModelLegacy::getInstance( 'attachment', 'otrsgatewaymodel' );
    ...
    function submit( $text = '', $subject = '', $priority = null, $queue = null, $ticketType = null, $token = false )
    {
        if ( !$this->_userID )
        {
            return array( 'error' => 'Permission denied' );
